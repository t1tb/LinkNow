    ...
    def POST(self):
        try:
			...
			if recMsg.Eventkey == 'lookFor':
                ...
                if courselst !=[]:
                    for i in ocrOut:
                    	if bytes(courselst[couter]).encode('utf-8') == 'None':
                    		content = content +bytes(i[0]) +" "+ bytes(i[1])
                        else :
			...
			for i, j in zip(course_list, url_list):
				#print('h =@@@@@@@@@@ ',i)
					...
					content += "<a href = '" + j + "'>👉" + i[0]+ ' ' + i[1]+ '</a>\n'
                			...
                			content +=  '⚠'+i[0]+ ' ' + i[1]+ '\n'
			content += '📌点击对应课程名称就可以加入课友群啦~\n📌如果点击没有出现二维码或是课名有⚠标记，请您检查一下是否输入正确\n📌如果确实输入正确，可能我们目前还没有对应的课友群，我们会尽快更新，感谢您的支持！'
